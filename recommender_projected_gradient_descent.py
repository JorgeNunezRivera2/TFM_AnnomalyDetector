...
def recommender_projected_gradient_descent(
    model_fn,
    users,
    deep_features,
    eps,
    ...
    if rand_init:
        eta = random_lp_vector(
            tf.shape(deep_features), norm, tf.cast(rand_minmax, deep_features.dtype), dtype=deep_features.dtype
        )
    else:
        eta = tf.zeros_like(deep_features)

    # Clip eta
    eta = clip_eta(eta, norm, eps)
    adv_x = deep_features + eta
    if clip_min is not None or clip_max is not None:
        adv_x = tf.clip_by_value(adv_x, clip_min, clip_max)

    if y is None:
        # Using model predictions as ground truth to avoid label leaking
        y = tf.argmax(model_fn(deep_features), 1)

    i = 0
    while i < nb_iter:
        adv_x = recommender_fast_gradient_method(
            model_fn,
            users,
            adv_x,
            eps_iter,
